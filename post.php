<?php
// Include file konfigurasi atau fungsi yang diperlukan
    session_start();
    include('function.php');

    if(isset($_SESSION['session_id_user'])){
        $id_user = $_SESSION['session_id_user'];
        $user_auth = $_SESSION['authority'];
        $user = readUserProfile($id_user);
      }else{
        $id_user = $_SESSION['session_id_guest'];
        $_SESSION['authority'] = 2;
        $user_auth = $_SESSION['authority'];
      }

    if(isset($_POST['btn-add'])){
        $id_mainchat = $_POST['id_mainchat'];
        $id_user = $_POST['id_user'];
        $isPostSucceed = createReply();
        if ($isPostSucceed > 0) {
            // jika penambahan sukses, redirect ke page post
            header("Location: post.php?id_mainchat=$id_mainchat");
        } else {
            echo "
            <script>
            alert('Post failed!');
            </script>
            ";
        }
    }
    $id_mainchat = $_GET['id_mainchat'];
    $data = readMainChatPost($id_mainchat);
    $countReply = countReplyChatPost($id_mainchat);
    $countLikesMain = countMainChatLikes($id_mainchat);
    $likeExist = checkLikesMain($id_user, $id_mainchat);
?>

<!DOCTYPE html>
<html lang="en">

<head>
  <meta charset="utf-8">
  <meta content="width=device-width, initial-scale=1.0" name="viewport">

  <title>Canvess</title>
  <meta content="" name="description">
  <meta content="" name="keywords">

  <!-- Favicons -->
  <link href="assets/img/canvesslogo.png" rel="icon">
  <link href="assets/img/apple-touch-icon.png" rel="apple-touch-icon">

  <!-- Google Fonts -->
  <link href="https://fonts.googleapis.com/css?family=Open+Sans:300,300i,400,400i,600,600i,700,700i|Raleway:300,300i,400,400i,500,500i,600,600i,700,700i|Poppins:300,300i,400,400i,500,500i,600,600i,700,700i" rel="stylesheet">

  <!-- Vendor CSS Files -->
  <link href="assets/vendor/aos/aos.css" rel="stylesheet">
  <link href="assets/vendor/bootstrap/css/bootstrap.min.css" rel="stylesheet">
  <link href="assets/vendor/bootstrap-icons/bootstrap-icons.css" rel="stylesheet">
  <link href="assets/vendor/boxicons/css/boxicons.min.css" rel="stylesheet">
  <link href="assets/vendor/glightbox/css/glightbox.min.css" rel="stylesheet">
  <link href="assets/vendor/swiper/swiper-bundle.min.css" rel="stylesheet">

  <!-- Template Main CSS File -->
  <link href="assets/css/gaya.css" rel="stylesheet">

  <!-- =======================================================
  * Template Name: MyResume
  * Updated: Mar 10 2023 with Bootstrap v5.2.3
  * Template URL: https://bootstrapmade.com/free-html-bootstrap-template-my-resume/
  * Author: BootstrapMade.com
  * License: https://bootstrapmade.com/license/
  ======================================================== -->
</head>

<body>

  <!-- ======= Mobile nav toggle button ======= -->
  <!-- <button type="button" class="mobile-nav-toggle d-xl-none"><i class="bi bi-list mobile-nav-toggle"></i></button> -->

  <header id="header" class="header d-flex align-items-center">

    <div class="container-fluid container-xl d-flex align-items-center justify-content-between">
      <a href="index.php" class="logo d-flex align-items-center">
        <!-- Uncomment the line below if you also wish to use an image logo -->
        <img src="assets/img/canvesslogo.png" alt="Canvess">
        <!-- <h1>Impact<span>.</span></h1> -->
      </a>
      <nav id="navbar" class="navbar">
        <ul>
          <li><a href="index.php">Home</a></li>
          <?php
            if($user_auth != 2){
              ?>
          <li class="dropdown"><a href="#"><?= $user['username'] ?> <i class="bi bi-chevron-down dropdown-indicator"></i></a>
          <ul>
              <li><a href="profile.php">Profile</a></li>
              <li><a href="settings.php">Settings</a></li>
            </ul>
          </li>
          <?php
            }else{
          ?>
          <li><a href="login.php?type=3&id_mainchat=<?=$id_mainchat?>">Login</a></li>
          <?php
            }
          ?>
        </ul>
      </nav><!-- .navbar -->

      <i class="mobile-nav-toggle mobile-nav-show bi bi-list"></i>
      <i class="mobile-nav-toggle mobile-nav-hide d-none bi bi-x"></i>

    </div>
  </header><!-- End Header -->
  <!-- End Header -->
  
  <main id="main">

<!-- ======= Portfolio Section ======= -->
<section id="contact" class="contact section-bg">
  <div class="container" data-aos="fade-up">

    <div class="row portfolio-container" data-aos="fade-up" data-aos-delay="200">
      <div class="portfolio-item">
        <div class="col">
            <div class="card">
                <div class="card-body text-center">
                <?php
                    if($data['isAnonymous'] == 0){
                      ?>
                      <h5 class="card-title">
                        <?php
                          if($data['id_user'] != $id_user){
                        ?>
                        <a href="seeProfile.php?username=<?=$data['username']?>" style="color:#272829">
                        <?php
                          }else{
                        ?>
                          <a href="profile.php" style="color:#272829">
                        <?php
                          }
                        ?>
                          <?= $data['username'] ?>
                        </a>
                      </h5>
                      <?php
                    }else{
                      ?> 
                      <h5 class="card-title">anonymous<?=$data['id_user']; ?></h5>
                      <?php
                    }
                    ?>
                <p class="card-text"><?= $data['main_message']; ?></p>
                <?php
                    if($data['id_user'] == $id_user){
                ?>
                <div class="text-center">
                    <a href="form/editPost.php?id_mainchat=<?=$data['id_mainchat']?>"><i class="bi bi-pen" style="color:#ffbb29"></i></a>
                    <a href="deletePost.php?id_mainchat=<?=$data['id_mainchat']?>&type=1" onclick="return confirm('You are going to delete a post')"><i class="bi bi-trash" style="color:red;"></i></a>
                </div>
                <?php } ?>
                </div>
                <div style="width:90%; margin: auto;">
                    <hr>
                </div>
                <div class="row justify-content-md-center" id="formContainer">
                    <?php
                    if($user_auth != 2){
                    ?>
                  <form id="likeForm" class="col-3 text-center like">
                    <input type="hidden" id="id_user" name="id_user" value="<?=$id_user?>">
                    <input type="hidden" id="id_mainchat" name="id_mainchat" value="<?=$id_mainchat?>">
                    <?php
                        if($likeExist == 0){
                    ?>
                    <button id="like-btn" class="like-btn" form="likeForm"><i class="bi bi-hand-thumbs-up"></i></button>
                    <?php
                        }else{
                    ?>
                    <button id="like-btn" class="like-btn liked" form="likeForm"><i class="bi bi-hand-thumbs-up"></i></button>
                    <?php
                        }
                    ?>
                    <span class="count"><?=$countLikesMain?></span>
                  </form>
                  <?php
                    }else{
                        ?>
                    <div id="likeForm" class="col-3 text-center like mb-3">
                        <button class="like-btn"><i class="bi bi-hand-thumbs-up"></i></button>
                        <span class="count"><?=$countLikesMain?></span>
                    </div>
                    <?php
                    }
                    ?>
                  <div class="col-3 text-center like">
                    <button><i class="bi bi-chat"></i></button>
                    <span><?=$countReply?></span>
                  </div>
                </div>
                <br>
                <div class="card-footer text-center">
                <small class="text-muted">Created on <?= $data['main_datetime']; ?></small>
                </div>
                <ul class="list-group list-group-flush">
                    <li class="list-group-item">
                        <?php
                            if($user_auth != 2){
                        ?>
                        <form action="post.php?id_mainchat=<?=$id_mainchat?>" method="POST" role="form" class="php-email-form" id="form-add">
                            <input type="hidden" name="id_mainchat" value="<?=$id_mainchat?>">
                            <input type="hidden" name="id_user" value="<?=$id_user?>">
                            <div class="form-check mt-1">
                                <label class="form-check-label radio-inline" for="anonymous">
                                    <input class="form-check-input" type="radio" name="isAnonymous" id="anonymous" value="1" checked>
                                    Reply as anonymous&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                </label>
                                <label class="form-check-label radio-inline" for="username">
                                    <input class="form-check-input" type="radio" name="isAnonymous" id="username" value="0">
                                    Reply as <?= $user['username'] ?>
                                </label>
                            </div>
                            <div class="row">
                                <div class="col col-lg-10 form-group mt-2">
                                    <input type="text" class="form-control" placeholder="Add a reply..." name="reply_message" id="reply_message" required>
                                </div>
                                <div class="col-md-auto mt-2">
                                    <div class="rep-btn"><button type="submit" name="btn-add" id="btn-add" form="form-add">Add reply</button></div>
                                </div>
                            </div>
                        </form>
                        <?php
                            }else{
                        ?>
                        <small class="text-muted"><a href="login.php?type=3&id_mainchat=<?=$id_mainchat?>">Login</a> to reply</small>
                        <?php
                            }
                        ?>
                    </li>
                    <?php
                        $replies = readReplyChatPost($id_mainchat);

                        foreach($replies as $reply){
                    ?>
                    <li class="list-group-item">
                    <?php
                    if($reply['isAnonymous'] == 0){
                      ?>
                      <h5 class="card-title">
                        <?php
                          if($reply['id_user'] != $id_user){
                        ?>
                        <a href="seeProfile.php?username=<?=$reply['username']?>" style="color:#272829">
                        <?php
                          }else{
                        ?>
                          <a href="profile.php" style="color:#272829">
                        <?php
                          }
                        ?>
                          <?= $reply['username'] ?>
                        </a>
                      </h5>
                      <?php
                        }else{
                          ?> 
                          <h5 class="card-title">anonymous<?=$reply['id_user']; ?></h5>
                          <?php
                        }
                      ?>
                        <?= $reply['reply_message'] ?>
                        <div class="text-center">
                        <?php
                            if($reply['id_user'] == $id_user){
                        ?>
                        <a href="deleteReply.php?id_replychat=<?=$reply['id_replychat']?>&id_mainchat=<?=$id_mainchat?>&type=2" onclick="return confirm('You are going to delete a reply')"><i class="bi bi-trash" style="color:red;"></i></a>
                        <?php
                            }
                        ?>
                        <small class="text-muted">Created on <?= $reply['reply_datetime']; ?></small>
                        </div>
                    </li>
                    <?php
                        }
                        ?>
                </ul>
            </div>
        </div>
        </div>
    </div>

  </div>
</section><!-- End Portfolio Section -->

</main><!-- End #main -->

<!-- ======= Footer ======= -->
<footer id="footer">
    <div class="container">
      <h3>Canvess</h3>
      <p>"You are never alone"</p>
      <div class="social-links">
        <a href="https://wa.me/6281282420569" class="whatsapp" target="_blank"><i class="bx bxl-whatsapp"></i></a>
        <a href="https://www.instagram.com/can.vess/" class="instagram" target="_blank"><i class="bx bxl-instagram"></i></a>
        <a href="mailto:mail.canvess@gmail.com" class="gmail" target="_blank"><i class="bx bxl-gmail"></i></a>
      </div>
      <div class="copyright">
      &copy; Copyright <strong>Canvess</strong>. All Rights Reserved
      </div>
      <div class="credits">
        <!-- All the links in the footer should remain intact. -->
        <!-- You can delete the links only if you purchased the pro version. -->
        <!-- Licensing information: [license-url] -->
        <!-- Purchase the pro version with working PHP/AJAX contact form: https://bootstrapmade.com/free-html-bootstrap-template-my-resume/ -->
        Designed by <a href="https://bootstrapmade.com/">BootstrapMade</a>
      </div>
    </div>
</footer><!-- End Footer -->

<div id="preloader"></div>
<a href="#" class="back-to-top back d-flex align-items-center justify-content-center"><i class="bi bi-arrow-up-short"></i></a>
<a href="form/createPost.php" class="create-post d-flex align-items-center justify-content-center"><i class="bi bi-plus"></i></a>


<!-- Vendor JS Files -->
<script src="assets/vendor/purecounter/purecounter_vanilla.js"></script>
<script src="assets/vendor/aos/aos.js"></script>
<script src="assets/vendor/bootstrap/js/bootstrap.bundle.min.js"></script>
<script src="assets/vendor/glightbox/js/glightbox.min.js"></script>
<script src="assets/vendor/isotope-layout/isotope.pkgd.min.js"></script>
<script src="assets/vendor/swiper/swiper-bundle.min.js"></script>
<script src="assets/vendor/typed.js/typed.min.js"></script>
<script src="assets/vendor/waypoints/noframework.waypoints.js"></script>

<!-- Template Main JS File -->
<script src="assets/js/main.js"></script>
<script src="assets/js/header.js"></script>
<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.6.4/jquery.min.js"></script>
<script type="text/javascript">
  $(document).ready(function() {
    const likeBtn = document.querySelectorAll('#like-btn');

  likeBtn.forEach(likeButton => {
    likeButton.addEventListener('click', (event) => {
        // Find the parent form of the clicked button
        var form = $('#like-btn').closest("form");

        var id_user = form.find("#id_user").val();
        var id_mainchat = form.find("#id_mainchat").val();
        var likeExist = form.find("#likeExist").val();

        var formData = {
            id_user: id_user,
            id_mainchat: id_mainchat,
            likeExist: likeExist
        };

        $.ajax({
            url: "likes.php", 
            type: "POST",
            data: formData,
            success: function(response) {
            }
        });
      });
    });
  });
</script>
  <script>
  // Ambil semua tombol "Like"
  const likeButtons = document.querySelectorAll('#like-btn');

  // Loop melalui setiap tombol "Like"
  likeButtons.forEach(likeButton => {
    // Tambahkan event listener untuk setiap tombol "Like"
    likeButton.addEventListener('click', (event) => {
      // Mencegah perilaku default dari tombol "Like" (menuju link lain)
      event.preventDefault();

      // Ambil elemen "Count" terkait
      const countElement = likeButton.nextElementSibling;
      // Ambil jumlah saat ini dari elemen "Count"
      let count = parseInt(countElement.textContent);

      // Periksa apakah tombol telah diklik sebelumnya atau tidak
      if (likeButton.classList.contains('liked')) {
        // Tombol telah diklik sebelumnya, kurangi jumlah "Count" dan reset
        count--;
        likeButton.classList.remove('liked');
      } else {
        // Tombol belum diklik sebelumnya, tambahkan 1 ke jumlah "Count" dan tandai tombol sebagai diklik
        count++;
        likeButton.classList.add('liked');
      }

      // Update nilai "Count" dengan jumlah yang baru
      countElement.textContent = count;
    });
  });
</script>

</body>
</html>
