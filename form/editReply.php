<?php
  ...
  if(isset($_SESSION['session_id_user'])){
    ...
    if(isset($_POST['btn-add'])){
      ...
      $isSucceed = updatePost($id_mainchat);
      if ($isSucceed > 0) {
    ...
    if(isset($_GET['id_replychat'])){
      $id_replychat = $_GET['id_replychat'];
      $chat = readReplyChatPost($id_mainchat);
    }
  }else{
...
<section id="contact" class="contact">
      <div class="container " data-aos="fade-up">
        ...
        <div class="row justify-content-md-center gx-lg-0 gy-4">
          ...
          <div class="col-lg-7">
            <form action="editPost.php" method="POST" role="form" class="php-email-form" id="form-add">
              <input type="hidden" name="id_mainchat" value="<?=$id_mainchat?>">
              <div class="form-group">
                <textarea class="form-control" id="froala" name="main_message" rows="7" maxlength="1000" placeholder="Write your story in 1000 characters..." required><?=$chat['main_message'] ?></textarea>
              </div>
              <div class="form-check mt-3">
                <?php
                  if($chat['isAnonymous'] == 1){
                ?>
              ...
              <div class="form-check mt-3">
                ...
                <?php }else{ ?>
                <input class="form-check-input" type="radio" name="isAnonymous" id="anonymous" value="1">
                <label class="form-check-label" for="anonymous">
                  Post as anonymous
                </label>
              </div>
              <div class="form-check mt-3">
                <input class="form-check-input" type="radio" name="isAnonymous" id="username" value="0" checked>
                <?php } ?>
                <label class="form-check-label" for="username">
                  Post as <?= $user['username'] ?>
                </label>
              </div>
              <div class="text-center"><button type="submit" name="btn-add" id="btn-add" form="form-add">Edit</button></div>
            </form>
